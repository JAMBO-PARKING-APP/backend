...
class DialogService {
  ...
  static void hideLoading() {
    if (_isLoadingShown) {
      ...
      final context = navigatorKey.currentContext;
      if (context != null) {
        // Safety: Only pop if the current navigator state has a modal route on top
        // This is tricky without more complex state tracking, but we'll use a more targeted pop
        try {
          Navigator.of(context, rootNavigator: true).pop();
          debugPrint('[DialogService] hideLoading: Popped successfully');
        } catch (e) {
          debugPrint('[DialogService] hideLoading Error: $e');
        }
      }
    } else {
      debugPrint('[DialogService] hideLoading called but loader not shown');
    }
  }
}
